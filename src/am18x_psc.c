...
static PSC_con_t* module_to_psc(psc_module_t module) {
...
am18x_rt psc_module_state_transition(psc_module_t module, uint32_t state) {
	...
	psc = module_to_psc(module);
	if (nr >= MODULE_NR_PER_PSC) {
		nr -= MODULE_NR_PER_PSC;
	}
	...
	psc->PTCMD = FIELD_SET(0, PTCMD_GO0, PTCMD_GO0);
